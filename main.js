...
.then(data => {
  ...
  // Render all radio stations initially
...
searchInput.addEventListener('keyup', function(event) {
  if (event.key === 'Enter' || event.keyCode === 13) {
    ...
    searchButton.click(); // Call searchButton function on Enter key press
  }
});

const homeButton = document.querySelector('#home-button');
homeButton.addEventListener('click', () => {
  filterRadioStations(''); // Call filterRadioStations function with empty query
  radioList.innerHTML = ''; // Clear the radio list
  renderRadioStations(radioStations); // Render all radio stations
});
...
searchButton.addEventListener('click', function(event) {
  ...
  const filteredStations = filterRadioStations(query);
  // Clear the radio list
  radioList.innerHTML = '';
  // Render the filtered radio stations
